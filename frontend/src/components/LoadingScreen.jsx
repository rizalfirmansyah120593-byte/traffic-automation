...
function LoadingScreen({ progress, onStop, url, options = {} }) {
  ...
  const urls = (Array.isArray(options.batchUrls) && options.batchUrls.length ? options.batchUrls : [url]).filter(Boolean).slice(0, 3);
  const activeUrl = progress?.currentUrl || url;
  const percentage = Math.min(100, Math.round(((completed + failed) / total) * 100));
  const step = remaining === 0 ? t('finalizing') : completed === 0 ? t('preparing') : t('processing');
  ...
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="loading-overlay">
    <motion.div initial={{ y: 24, scale: .97 }} animate={{ y: 0, scale: 1 }} className="loading-panel">
      ...
      <div className="relative">
        <div className="mb-7 flex items-start justify-between"><div className="flex items-center gap-3"><div className="relative flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500 to-fuchsia-500 shadow-lg shadow-violet-500/30"><motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: 'linear' }} className="h-7 w-7 rounded-full border-2 border-white/30 border-t-white" /></div><div><p className="text-xs font-bold uppercase tracking-[.2em] text-violet-300">{t('live')}</p><h2 className="text-2xl font-bold">{t('inProgress')}</h2></div></div><div className="flex items-center gap-1.5 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-emerald-300"><span className="h-2 w-2 animate-pulse rounded-full bg-emerald-400" /> {t('active')}</div></div>
        ...
        <div className="mb-5 grid grid-cols-3 gap-2.5">{stats.map(([label, value, color, bg], index) => { const labels = [t('completed'), t('remaining'), t('failed')]; return <div key={label} className={`rounded-2xl border border-white/10 ${bg} p-3`}><div className={`text-2xl font-bold ${color}`}>{value}</div><div className="mt-1 text-[9px] font-bold uppercase tracking-widest text-slate-500">{labels[index]}</div></div>; })}</div>
        <div className={`mb-6 grid gap-3 ${urls.length > 1 ? 'md:grid-cols-2 xl:grid-cols-3' : 'grid-cols-1'}`}>
          {urls.map((target, index) => {
            const isActive = target === activeUrl;
            const isDone = Number(progress?.completedUrls || 0) > index;
            return <div key={`${target}-${index}`} className={`rounded-2xl border p-4 transition-all ${isActive ? 'border-violet-400/60 bg-violet-500/15 shadow-lg shadow-violet-950/20' : isDone ? 'border-emerald-400/30 bg-emerald-500/10' : 'border-white/10 bg-white/[.04]'}`}>
              <div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-500"><FaGlobe className={isActive ? 'text-violet-300' : 'text-cyan-400'} /> {t('target')} {index + 1}</div><span className={`text-[9px] font-bold uppercase tracking-wider ${isActive ? 'text-violet-300' : isDone ? 'text-emerald-300' : 'text-slate-500'}`}>{isActive ? t('active') : isDone ? t('completed') : 'Queued'}</span></div>
              <p className="break-all font-mono text-xs leading-5 text-slate-200">{target}</p>
              {isActive && <div className="mt-3 h-1 overflow-hidden rounded-full bg-slate-800"><motion.div animate={{ width: `${percentage}%` }} className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-violet-500" /></div>}
            </div>;
          })}
          <div className="rounded-2xl border border-white/10 bg-white/[.04] p-4"><div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-500"><FaNetworkWired className="text-blue-400" /> {t('clientIp')}</div><p className="font-mono text-xs text-slate-200">{ip}</p></div>
        </div>
        ...
        <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: .98 }} onClick={onStop} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-rose-600 to-red-500 py-4 text-sm font-bold uppercase tracking-widest shadow-lg shadow-red-950/30"><FaStop className="text-xs" /> {t('stop')}</motion.button><p className="mt-3 text-center text-[10px] text-slate-600">{t('safeStop')}</p>
      </div>
    </motion.div>
  </motion.div>;
}
